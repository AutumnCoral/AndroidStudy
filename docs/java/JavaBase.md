...
public class StringDemo {
   public static void main(String[] args) {
      String a = "Hello"; //a为一个类类型的对象，"Hello"为一个对象，字符串最大的特点就是一但初始化就不会在改变
      String b = "Hell";
      String c = new String("Hello");// a和c的区别：a在内存中只有一个对象，c在内存中有两个对象

      /*
       *   charAt:返回给定位置的代码单元:
       *   codePointAt:返回从给定位置开始的码点。
       *   offsetByCodePoints:返回从startIndex代码点开始，位移cpCount后的码点索引。
       *   compareTo:按照字典顺序，如果字符串位于other之前，返回一个负数；如果字符串位于other之后，返回一个正数；如果两个字符串相等，返回0。
       *    startsWith,endsWith:如果字符串以suffix开头或结尾，则返回true。
      ...
      System.out.println("index0f(String str):" + a.indexOf("H"));//输出0 返回H所在的下标，从0开始计算，如果返回-1，表示该str在字符串中不存在，所以，也可以用于判断是否存在于字符串中if( a.indexOf("H") != -1)
      System.out.println("index0f(String str, int fromIndex):" + a.indexOf("e", 2));//输出-1 返回e所在的第一个子串的开始位置,从2开始计算
...
[优秀博客](https://blog.csdn.net/petterp/article/details/89043847)
...
某一对象所持有的属性和方法，在不同类的类中调用具有不同的执行逻辑，就是该属性和方法具有不确定性，
